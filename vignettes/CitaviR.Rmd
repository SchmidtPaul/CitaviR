...
At this point we have already gained information and could continue with steps 4 and 5. However, sometimes duplicates hold different information as it is the case here for `ct_02` and the columns `PubMed ID` and `DOI name`:

```{r}
CitDat[2:3, c("clean_title_id", "obv_dup_id", "Title", "PubMed ID", "DOI name")]
```
...
```{r}
CitDat <- CitDat %>% 
  handle_obvious_dups(fieldsToHandle = c("PubMed ID", "DOI name"))
